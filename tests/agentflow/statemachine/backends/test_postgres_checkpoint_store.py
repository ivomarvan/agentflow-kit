...
Requires: docker compose up -d  (postgres service)
Run with: pytest -m integration

Environment:
    POSTGRES_DSN: override default connection string
                  default: postgresql://agentflow:agentflow@localhost:5432/agentflow
"""
...
_DEFAULT_DSN = "postgresql://agentflow:agentflow@localhost:5432/agentflow"
...
@pytest.fixture
async def store() -> AsyncGenerator[PostgresCheckpointStore, None]:
    """Connected PostgresCheckpointStore, torn down after each test."""
    dsn = os.environ.get("POSTGRES_DSN", _DEFAULT_DSN)
    async with PostgresCheckpointStore(dsn, state_factory=_state_factory) as s:
        yield s


@pytest.mark.integration
@pytest.mark.asyncio
async def test_save_and_load_roundtrip(store: PostgresCheckpointStore) -> None:
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_list_steps(store: PostgresCheckpointStore) -> None:
    """Save 3 steps and verify list_steps returns them sorted."""
    run_id = f"test_{uuid.uuid4().hex[:8]}"
    for step in [3, 1, 2]:
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_load_missing_raises_keyerror(store: PostgresCheckpointStore) -> None:
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_overwrite_same_step(store: PostgresCheckpointStore) -> None:
